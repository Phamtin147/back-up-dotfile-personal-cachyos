...
Singleton {
  ...
  property var resolvedAntigravityPaths: []
  ...
  readonly property var applications: [
    ...
    },
    {
      ...
      "clients": [
        ...
        {
          "name": "antigravity",
          "path": "~/.antigravity/extensions/noctalia.noctaliatheme-0.0.5-universal/themes/NoctaliaTheme-color-theme.json"
        }
      ]
    },
    {
  ...
  function resolvedCodeClientPaths(clientName) {
    ...
    if (clientName === "antigravity")
      return resolvedAntigravityPaths;
    return [];
  }
  ...
  readonly property var codeClients: {
    ...
    if (codeApp && codeApp.clients) {
      codeApp.clients.forEach(client => {
                                ...
                                } else if (client.name === "antigravity") {
                                  // For AntiGravity IDE: ~/.antigravity/extensions/... -> ~/.antigravity
                                  baseConfigDir = "~/.antigravity";
                                }
                                clients.push({
                                               "name": client.name,
  ...
  Process {
    ...
    stdout: SplitParser {
      onRead: data => {
        var line = data.trim();
        if (line)
        antigravityResolverProcess.paths.push(line);
      }
    }
    onExited: {
      root.resolvedAntigravityPaths = paths;
    }
  }
